...
df <- data.frame(rbind(
    ...
    cbind(unlist(gravy_other), "other"),
    cbind(unlist(gravy_tp), "two phase")))
colnames(df) <- c("score", "type") 
df$score <- as.numeric(df$score)
df$type <- factor(df$type, levels = c("other", "two phase", "shared")) 
comparisons <- list( c("other", "shared"), c("two phase", "shared"))
...
df <- data.frame(rbind(
    ...
    cbind(unlist(iep_other), "other"),
    cbind(unlist(iep_tp), "two phase")))
colnames(df) <- c("score", "type") 
df$score <- as.numeric(df$score)
df$type <- factor(df$type, levels = c("other", "two phase", "shared")) 
comparisons <- list( c("other", "shared"), c("two phase", "shared"))
...
## GRAVY score
rD <- rowData(serum)
gravy <- lapply(rD[rownames(tT), "feature"], function(aa) calculateGravyScore(AAString(aa)))
gravy <- unlist(gravy)
df <- data.frame(gravy = gravy, t = t)
ggplot(df, aes(x = gravy, y = t)) + 
    geom_point(alpha = 0.3) + 
...
## isoelectric point
iep <- lapply(rD[rownames(tT), "feature"], function(aa) calculateIsoelectricPoint(AAString(aa), method = "IPC_protein"))
iep <- unlist(iep)
df <- data.frame(iep = iep, t = t)
ggplot(df, aes(x = iep, y = t)) + 
    geom_point(alpha = 0.3) + 
